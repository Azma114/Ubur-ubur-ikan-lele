...
def save_data(data):
    ...
    try:
        with open(DATA_FILE, mode='w', newline='') as file:
            fieldnames = ['NoPegawai', 'NamaPegawai', 'TanggalAwal', 'TanggalAkhir', 'GajiPerHari']
...
# Fungsi untuk menampilkan data gaji pegawai dalam bentuk tabel
def view_data(data):
    """Display the salary data in a formatted table."""
    if not data:
        print("Data gaji pegawai kosong.")
        return
    print(f"{'NoPegawai':<12} {'NamaPegawai':<20} {'TanggalAwal':<12} {'TanggalAkhir':<12} {'GajiPerHari':<15} {'TotalGaji':<12}")
    print("-" * 85)
    for row in data:
        total_gaji = calculate_total_gaji(row['TanggalAwal'], row['TanggalAkhir'], float(row['GajiPerHari']))
        print(f"{row['NoPegawai']:<12} {row['NamaPegawai']:<20} {row['TanggalAwal']:<12} {row['TanggalAkhir']:<12} {row['GajiPerHari']:<15} {total_gaji:<12.2f}")
...
# Fungsi untuk mengedit data pegawai yang sudah ada
def edit_data(data):
...
# Fungsi untuk menghapus data pegawai berdasarkan NoPegawai
...
# Fungsi untuk mencari data berdasarkan NoPegawai atau NamaPegawai
...
# Fungsi untuk menghitung total gaji berdasarkan tanggal dan gaji per hari
...
# Fungsi untuk memvalidasi urutan tanggal (akhir >= awal)
...
# Fungsi untuk memvalidasi angka gaji per hari
...
# Eksekusi program dimulai dari sini
